...
import { boot } from "../../routes/backend/routesBackend.js";
...
const backendClientService = {
  ...
  publish(topic, payload, options) {
    return tr.run(() => backendProxy.publish(topic, payload, options));
  },
  subscribe(topic, options, listener) {
    return tr.run(() => backendProxy.subscribe(topic, options, listener));
  },
};

export { backendClient, backendProxy, backendClientService };
